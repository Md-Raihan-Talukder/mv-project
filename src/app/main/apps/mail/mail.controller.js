(function () {
    ...
    /** @ngInject */
    function MailController($scope, $rootScope, $document, $mdDialog, $mdMedia, $mdSidenav, $state, msApi, mailService) {
        var vm = this;
        ...
        vm.AddPOLabel = AddPOLabel;

        vm.loadPOrelatedMail = loadPOrelatedMail;
        ...
        function loadPOrelatedMail(type, filter) {
        ...
        function AddPOLabel(type, labelText) {
            if (vm.currentThread) {
                vm.currentThread[type] = labelText;
                return;
            }

            if (vm.selectedThreads.length > 0) {
                for (var i = 0; i < vm.selectedThreads.length; i++) {
                    vm.selectedThreads[i][type] = labelText;
                }
            }
        }
        ...
        function addNewLabels(name) {
            var folder = {
                "id": vm.labels.length + 1,
                "name": name,
                "title": name,
                "color": "md-green-600-bg"
            }

            vm.labels.push(folder);
